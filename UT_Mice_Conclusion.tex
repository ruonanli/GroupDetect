...
\todd{Add to the beginning of Section 4, ``In all cases we use four-level temporal pyramids for the interactions, and we set the time unit to be half the duration of the cells in the lowest level.'' Then remove these words from elsewhere in the section.} 
...
\begin{table}[h]
\centering \caption{Accuracies for the proposed method and the baselines on Caltech Resident-Intruder Mouse Dataset. (\%) \todd{Only show the Trajectory\_2 results in this table, and do not include the discussion about different ways to use trajectory features. Instead put this bigger table in the supplemental material along with that discussion.}}
\footnotesize{
